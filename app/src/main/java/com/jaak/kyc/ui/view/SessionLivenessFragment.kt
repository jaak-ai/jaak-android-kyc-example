...
import com.jaak.kyc.databinding.FragmentSessionLivenessBinding
...
class SessionLivenessFragment : Fragment() {
    ...
    private fun populateUI(state: SessionDetailState.Success) {
        ...
        if (!summary.photo.isNullOrEmpty()) {
            ...
            com.squareup.picasso.Picasso.get()
                .load(summary.photo)
                .placeholder(com.jaak.kyc.R.drawable.ic_person) // Mientras carga
                .error(com.jaak.kyc.R.drawable.ic_person) // Si falla
                .into(binding.ivUserPhoto)
        } else {
            android.util.Log.d("SessionLivenessFragment", "Photo URL es null o vacío")
            binding.ivUserPhoto.setImageResource(com.jaak.kyc.R.drawable.ic_person)
        }

        // Nombre completo
        val fullName = "${summary.name ?: ""} ${summary.lastName ?: ""}".trim()
        binding.tvUserName.text = fullName.ifEmpty { getString(com.jaak.kyc.R.string.user_no_name) }

        // Tiempo total
        val timeText = if (summary.totalTime != null) {
            getString(com.jaak.kyc.R.string.liveness_total_time_format, formatDuration(summary.totalTime))
        } else {
            getString(com.jaak.kyc.R.string.liveness_total_time_na)
        }
        ...
        binding.tvTotalTimeDetail.text = if (summary.totalTime != null) {
    ...
    private fun showLocationDialog(latitude: Double, longitude: Double) {
        AlertDialog.Builder(requireContext())
            .setTitle(getString(com.jaak.kyc.R.string.location_dialog_title))
            .setMessage(getString(com.jaak.kyc.R.string.location_dialog_message))
            .setPositiveButton(getString(com.jaak.kyc.R.string.location_google_maps)) { _, _ ->
                openGoogleMaps(latitude, longitude)
            }
            .setNegativeButton(getString(com.jaak.kyc.R.string.cancel), null)
            .show()
    }
