package ai.jaak.kyc.domain.usecase
...
import ai.jaak.kyc.domain.service.*
...
class ProcessManagementUseCase @Inject constructor(
        ...
        return try {
            ...
            if (authToken == null && isOnlineMode) {
                val error = processErrorManager.logCustomError(
                    ...
                    category = ai.jaak.kyc.data.local.entity.ErrorCategory.AUTHENTICATION,
                    severity = ai.jaak.kyc.data.local.entity.ErrorSeverity.HIGH,
                    showToUser = true,
            ...
            if (result.isSuccess) {
                ...
                val error = if (exception != null) {
                    ...
                    processErrorManager.logCustomError(
                        ...
                        category = ai.jaak.kyc.data.local.entity.ErrorCategory.BUSINESS_LOGIC,
                        severity = ai.jaak.kyc.data.local.entity.ErrorSeverity.HIGH
                    )
                }
    ...
    suspend fun retryFailedService(processId: String, serviceType: KycServiceType): RetryResult {
        ...
        return when (retryResult) {
            is ai.jaak.kyc.domain.service.RetryResult.CanRetry -> {
            ...
            is ai.jaak.kyc.domain.service.RetryResult.MustWait -> {
                RetryResult.MustWait(retryResult.waitTimeMs)
            }
            is ai.jaak.kyc.domain.service.RetryResult.NoRetryNeeded -> {
    ...
    fun getProcessStatusFlow(processId: String): Flow<ProcessStatus> {
        return combine(
            ...
            val criticalErrors = unresolvedErrors.filter { 
                it.severity == ai.jaak.kyc.data.local.entity.ErrorSeverity.CRITICAL 
            }
    ...
    private fun calculateProgress(serviceStates: List<ai.jaak.kyc.data.local.entity.ServiceExecutionState>): ProcessProgress {
        val totalServices = serviceStates.size
        val completedServices = serviceStates.count { 
            it.status == ai.jaak.kyc.data.local.entity.ServiceStatus.COMPLETED || 
            it.status == ai.jaak.kyc.data.local.entity.ServiceStatus.SYNCED 
        }
        val failedServices = serviceStates.count { 
            it.status == ai.jaak.kyc.data.local.entity.ServiceStatus.FAILED 
        }
