...
class FacialVerificationInstructionsActivity : AppCompatActivity(), VisageListener {
    ...
    private fun initFaceDetector() {
        // Inicializar VisageSDK con licencia dinámica del header x-trace-id
        val visageLicense = ai.jaak.kyc.utils.FlowLicenseManager.getLicense(this)
        if (visageLicense.isNullOrEmpty()) {
            android.util.Log.e("FacialVerification", "❌ No hay licencia disponible. Debe obtenerla del header x-trace-id de la sesión.")
            android.widget.Toast.makeText(this, "Error: No se encontró licencia válida", android.widget.Toast.LENGTH_LONG).show()
            finish()
            return
        }
        val visageEnvironment = profileManager.getVisageEnvironment()
        android.util.Log.d("FacialVerification", "→ Inicializando VisageSDK con licencia: $visageLicense, ambiente: $visageEnvironment")
        VisageSDK.initialize(visageLicense)
        VisageSDK.setEnvironment(visageEnvironment, this)
