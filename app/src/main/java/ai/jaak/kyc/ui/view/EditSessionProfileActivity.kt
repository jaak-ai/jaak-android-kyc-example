...
import ai.jaak.kyc.databinding.ActivityEditSessionProfileBinding
...
class EditSessionProfileActivity : AppCompatActivity() {
    ...
    @Inject
    lateinit var kycOfflineRepository: ai.jaak.kyc.data.repository.KycOfflineRepository
    ...
    private var mosaicAdapter: ai.jaak.kyc.ui.adapter.MosaicModuleAdapter? = null
    private val mosaicModules = ai.jaak.kyc.data.model.MosaicModule.getDefaultModules().toMutableList()
    ...
    private fun setupMosaicModules() {
        if (mosaicAdapter == null) {
            ...
            mosaicAdapter = ai.jaak.kyc.ui.adapter.MosaicModuleAdapter(mosaicModules) {
            ...
            try {
                ...
                when (selectedFlowType) {
                    ...
                    "MOSAIC" -> {
                        ...
                        val config = ai.jaak.kyc.data.model.MosaicConfig(modules, shortKey)
                        val mosaicUrl = config.buildMosaicUrl(profileManager.getCurrentMosaicUrl())
                        ...
                        Log.d("EditSessionProfile", "Header x-trace-id: $xTraceId")

                        // Guardar licencia (x-trace-id con "L" al inicio)
                        if (!xTraceId.isNullOrEmpty()) {
                            val license = "L$xTraceId"
                            ai.jaak.kyc.utils.FlowLicenseManager.saveLicense(this@EditSessionProfileActivity, license)
                            Log.d("EditSessionProfile", "✓ Licencia guardada: $license")
                        } else {
                            Log.w("EditSessionProfile", "⚠ No se encontró header x-trace-id en la respuesta")
                        }
