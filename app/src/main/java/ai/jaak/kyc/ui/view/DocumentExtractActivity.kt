...
import ai.jaak.kyc.databinding.ActivityDocumentExtractBinding
...
class DocumentExtractActivity : AppCompatActivity() {
    ...
    private fun loadDocumentExtractData() {
        ...
        val extractData = intent.getParcelableExtra<ai.jaak.kyc.data.model.DocumentExtractData>("extract_data")
     ...
     */
    private fun parseExtractedData(extractData: ai.jaak.kyc.data.model.DocumentExtractData) {
    ...
    private fun parseEstadosOcr(evaluation: Map<*, *>, thresholdsJson: String?, validationJson: String?) {
        try {
            val estadosOcrList = mutableListOf<ai.jaak.kyc.ui.adapter.EstadoOcrItem>()
            ...
            ocrStates?.forEach { (key, value) ->
                if (value is Boolean) {
                    estadosOcrList.add(
                        ai.jaak.kyc.ui.adapter.EstadoOcrItem(
            ...
            if (estadosOcrList.isNotEmpty()) {
                ...
                binding.rvEstadosOcr.apply {
                    ...
                    adapter = ai.jaak.kyc.ui.adapter.EstadoOcrAdapter(estadosOcrList, showHeader = true)
                }
            } else {
